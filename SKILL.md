...
2. 比较远程最新 commit 的 `sha` 与本地记录的版本（当前 `v2.7.0`）。
...
> ⚠️ **重要实测发现：** Agnes 图像模型只有 3 个固定输出档位。请求"标准2K"尺寸（如 2560x1440、1920x1080 等）会被降级到 1K（1312x736）。详见下方说明。

| 比例 | 2K（精确输出） | 4K | 说明 |
|------|---------------|-----|------|
| 4:3 | 1024x768 | — | 标准横版（唯一精确1K） |
| 3:4 | 768x1024 | — | 标准竖版（唯一精确1K） |
| 16:9 | **2624x1472** | 3840x2160 | 宽屏横版（唯一精确2K，宽高都是16的倍数） |
| 9:16 | 1440x2560 | 2160x3840 | 宽屏竖版（默认，映射到 2.5K 级） |

> ⚠️ **Agnes 图像模型分辨率映射规则（实测）：**
>
> Agnes 内部只有 3 个固定输出档位：
> 1. **1K 档：1312x736**（~1MP，宽高都是16的倍数）—— 所有 <2624x1472 的请求都会被降级到此
> 2. **2K 档：2624x1472**（~3.9MP，宽高都是16的倍数）—— 仅当精确请求 2624x1472 时原尺寸输出
> 3. **4K 档：3848x2160**（~8.3MP）—— 请求 3840x2160 时映射输出
>
> **实测降级示例：**
> - 请求 1280x720 → 输出 1312x736（1K）
> - 请求 1920x1080 → 输出 1312x736（1K）
> - 请求 2048x1152 → 输出 1312x736（1K）
> - 请求 2304x1296 → 输出 1312x736（1K）
> - 请求 2560x1440 → 输出 1312x736（1K）
> - 请求 2720x1536 → 输出 2728x1528（超大但非16倍数）
> - 请求 2880x1620 → 输出 2880x1616（宽高都是16倍数，但高-4像素）
> - 请求 3840x2160 → 输出 3848x2160（4K，宽+8像素）
>
> **推荐策略：** 如需 16:9 的2K分辨率且保证宽高都是16的倍数，**必须**使用 **2624x1472**。
