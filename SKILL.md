...
- `size` 格式错误 → 只接受 `1024x768` / `1024x1024` / `768x1024` → 改为正确格式重试

**视频模型 400 → 按以下分支排查：**
- 必填：`model` + `prompt` → 缺任一项 → 补齐后重试
- `num_frames` 不满足约束 → 必须 ≤ 441 且满足 `8n + 1` → 改为有效值（81/121/161/241/441）
...
1. 检查查询接口 → 是 `GET /agnesapi?video_id=<ID>` 吗？
   ...
   - **是 task_id →** 🔴 **这是根因！** 立即切换为 video_id 查询方式：
     ```
     ❌ 错误：GET /v1/videos/{task_id}
     ✅ 正确：GET /agnesapi?video_id=<ID>
     ```
2. 确认 video_id 获取方式 → 创建任务后，响应体中取 `video_id` 字段
3. 轮询间隔：5 秒一次，不要频繁轮询
...
| 3 | 用 `task_id` 查询视频结果 | 用 `video_id` 查询：`GET /agnesapi?video_id=<ID>` | 视频排队异常延长（>5分钟） |
...
| 8 | `num_frames` 使用任意值 | 只用 8n+1 值：81/121/161/241/441 | 400 Bad Request |
...
### 10.1 Yacey 的 Agnes AI 生成 Skill（推荐 Agent 用户）
...
### 10.2 kangarooking 的免费模型 Skills（推荐 Codex 用户）
...
### 10.3 16nic 的 ComfyUI 节点（推荐 ComfyUI 用户）
...
### 11.2 OpenAI Codex CLI 配置（命令行用户）
