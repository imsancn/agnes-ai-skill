...
2. 比较远程最新 commit 的 `sha` 与本地记录的版本（当前 `v2.7.1`）。
...
> ⚠️ **重要实测发现：** Agnes 图像模型只有 6 个固定输出档位。请求"标准2K"尺寸（如 2560x1440、1920x1080、2048x1536 等）会被降级到 1K。详见下方说明。

| 比例 | 1K | 2K（精确输出） | 4K | 说明 |
|------|----|---------------|-----|------|
| 4:3 | 1024x768 | — | — | 标准横版（1K精确） |
| 3:4 | 768x1024 | — | — | 标准竖版（1K精确） |
| 16:9 | 1312x736 | **2624x1472** | 3840x2160 | 宽屏横版（2K/4K精确输出，宽高都是16的倍数） |
| 9:16 | 736x1312 | 1472x2624 | 2160x3840 | 宽屏竖版（默认9:16，映射到 2.5K 级） |
| 1:1 | — | **2048x2048** | — | 正方形（2K精确输出，宽高都是16的倍数） |

> ⚠️ **Agnes 图像模型分辨率映射规则（实测）：**
>
> Agnes 内部有 6 个固定输出档位：
> 1. **1K 4:3 档：1152x864**（~1MP）—— 4:3 族的 2K 请求（如 2048x1536）会被降级到此
> 2. **1K 3:4 档：864x1152**（~1MP）—— 3:4 族的 2K 请求（如 1536x2048）会被降级到此
> 3. **1K 16:9 档：1312x736**（~1MP，宽高都是16的倍数）—— 所有 <2K 的16:9 请求都会被降级到此
> 4. **2K 1:1 档：2048x2048**（~4.2MP，宽高都是16的倍数）—— 仅当精确请求 2048x2048 时原尺寸输出
> 5. **2K 16:9 档：2624x1472**（~3.9MP，宽高都是16的倍数）—— 仅当精确请求 2624x1472 时原尺寸输出
> 6. **4K 16:9 档：3848x2160**（~8.3MP）—— 请求 3840x2160 时映射输出
>
> **实测降级示例：**
> - 请求 1280x720 → 输出 1312x736（1K 16:9）
> - 请求 1920x1080 → 输出 1312x736（1K 16:9）
> - 请求 2048x1152 → 输出 1312x736（1K 16:9）
> - 请求 2560x1440 → 输出 1312x736（1K 16:9）
> - 请求 2048x1536 → 输出 1152x864（1K 4:3）
> - 请求 1536x2048 → 输出 864x1152（1K 3:4）
> - 请求 2720x1536 → 输出 2728x1528（超大但非16倍数）
> - 请求 2880x1620 → 输出 2880x1616（宽高都是16倍数，但高-4像素）
> - 请求 3840x2160 → 输出 3848x2160（4K，宽+8像素）
>
> **推荐策略：** 如需 16:9 的2K分辨率且保证宽高都是16的倍数，**必须**使用 **2624x1472**；如需 1:1 正方形2K，**必须**使用 **2048x2048**。
