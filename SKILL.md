...
description: |
  ...
  生图、生视频、图生图、文生图、图生视频、文生视频、灰度自检、1M 上下文、4K 图片、4K 图、4K、灰度、自检、反馈、Bug 反馈、问题反馈、错误码。
...
2. 比较远程最新 commit 的 `sha` 与本地记录的版本（当前 `v2.3.0`）。
...
> **当前规格：** Agnes-2.0-Flash 上下文窗口参考值为 **512K**，Max Output **64K**。
>
> - Context：**512K**（⚠️ **内测功能**，来源为社区实测反馈，[官方 MODEL_CATALOG.md](https://github.com/AgnesAI-Labs/Agnes-AI/blob/main/MODEL_CATALOG.md) 仍标注为 256K，尚未同步更新）
> - Max Output：64K
>
> **使用建议：** 默认可尝试 512K 上下文；如遇到与上下文长度相关的错误（如 413/400），请回退到官方标注的 **256K** 作为上限，等待官方正式公告后再使用更大窗口。
>
> 1.5 Flash 保持 256K/64K。
...
- RPM 超限 → 检查请求频率（免费 20/min，详见第 5 节）
- 超限 → 实现客户端队列，请求间隔 ≥ 3s
...
- Context：512K（⚠️ 内测，官方文档仍为 256K，遇上下文错误请回退 256K）
- Max Output：64K
...
> 以下 RPM 和配额为 **2026-06-22 官方公开参考值**，可能随定价或容量调整而变化。详细的模型级 RPM 表请查看 [官方 MODEL_CATALOG.md](https://github.com/AgnesAI-Labs/Agnes-AI/blob/main/MODEL_CATALOG.md)，以官方平台控制台为最终依据。
...
|------|-----:|----------------|--------------------------|------------------|
| Starter | $4 | 1,500 请求/5小时；15,000 请求/周 | 4,000 张/天 | 500 秒/天 |
| Plus | $10 | 7,500 请求/5小时；75,000 请求/周 | 4,000 张/天 | 500 秒/天 |
| Pro | $50 | 30,000 请求/5小时；300,000 请求/周 | 4,000 张/天 | 500 秒/天 |
...
| 429 | 速率限制（详见第 5 节） | RPM 超限、并发请求过多、自动重试频率过快 | 等待后重试，降低频率和并发，使用指数退避 |
...
| 429 | 速率限制 | 请求过于频繁 | 降低频率，实现退避重试（详见第 5 节） |
...
| 9 | 429 后立即重试 | 等待 ≥ 3s 后重试，实现指数退避 | 持续被限流 |
...
>
> **⚠️ 重要：1M 上下文测试是"尽力而为"的方法，结果仅供参考。灰度分配是动态的，可能随时变化。4K 图片无需灰度测试，已全面支持。**
...
>
> **注意**：agnes-2.0-flash Context 已开放至 512K（内测，官方文档未同步），因此 1M 灰度测试需要发送 **超过 512K tokens**（约 **2M+ 字符**）的 payload。如 512K 已满足需求，无需测试 1M。
...
> **注意：** 1M 上下文灰度是动态分配的，不支持时无需担心，未来会逐步全量开放。以上测试仅供参考，实际以官方公告为准。4K 图片已全面开放，无需灰度测试。
